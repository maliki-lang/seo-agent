...
class TrustSignalAnalyzer:
    ...
    TESTIMONIAL_PATTERNS = {
        ...
        'with_company': [
            r'([A-Z][a-z]+(?:\s+[A-Z]\.?)?),\s*([A-Za-z\s]+(?:Co\.|Inc\.|LLC|Agency|Studio|Clinic))',
        ]
    }
    ...
    SOCIAL_PROOF_PATTERNS = {
        'customer_count': [
            r'(\d{1,3}(?:,\d{3})*\+?)\s*(?:customers?|customers?|users?|creators?|businesses?|shows?)',
            r'(?:trusted|used|loved)\s+by\s+(\d{1,3}(?:,\d{3})*\+?)',
            r'(?:join|over)\s+(\d{1,3}(?:,\d{3})*\+?)\s*(?:customers?|customers?)',
    ...
    AUTHORITY_PATTERNS = {
        ...
        'awards': [
            ...
            r'(?:best|top|leading)\s+(?:footwear|hosting|software|platform|tool)',
