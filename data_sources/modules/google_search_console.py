...
class GoogleSearchConsole:
    ...
    def __init__(
        ...
        Args:
            site_url: Search Console property, e.g. "sc-domain:sunnystep.com" (Domain
                property) or "https://www.example.com/" (URL-prefix property)
            credentials_path: Path to credentials JSON
            country: ISO-3 country code to filter every query by (e.g. "sgp").
                Empty = all countries.
        """
        # GSC_PROPERTY is the exact property name the API expects. GSC_SITE_URL is kept
        # as a fallback because other scripts derive the site domain from it.
        self.site_url = site_url or os.getenv('GSC_PROPERTY') or os.getenv('GSC_SITE_URL')
        self.country = (country if country is not None else os.getenv('GSC_COUNTRY', '')).strip().lower()
        credentials_path = credentials_path or os.getenv('GSC_CREDENTIALS_PATH')

        if not self.site_url:
            raise ValueError("GSC_PROPERTY (or GSC_SITE_URL) must be provided or set in environment")
    ...
    def _query(self, request: Dict[str, Any]) -> Dict[str, Any]:
        """Run a Search Analytics query, applying the country filter if one is set."""
        if self.country:
            country_filter = {'dimension': 'country', 'operator': 'equals', 'expression': self.country}
            groups = request.get('dimensionFilterGroups') or [{'filters': []}]
            for group in groups:
                group.setdefault('filters', []).append(country_filter)
    ...
    def _calculate_commercial_intent(self, keyword: str) -> float:
        ...
        medium_intent = [
            ...
            'shoes', 'footwear', 'comfort', 'arch support', 'walking', 'marketing'
        ]
    ...
    def get_page_performance(
        ...
        keywords_response = self._query(keywords_request)
    ...
    def get_trending_queries(
        ...
        recent_response = self._query(recent_request)
        ...
        comparison_response = self._query(comparison_request)
