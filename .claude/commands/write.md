...
- **Technical Accuracy**: Ensure terminology and processes are correct for the footwear industry
...
Cluster: [comfortable shoes (discovery) | foot comfort education & arch support | everyday flats & loafers | work / office comfort shoes | comfortable walking shoes (pillar)]
Awareness Stage: [unaware | problem-aware | solution-aware | product-aware | ready-to-buy]
...
## Eval Gate — Control Tower (REQUIRED, authoritative)

After the content-scorer loop, the draft MUST pass the company eval gate. This is
the enforced, code-level gate — not a checklist. It maps the deterministic
scorers onto the 9-dimension company scorecard, runs the **zero-tolerance YMYL
compliance gate**, and (for foot-health topics especially) requires an
independent critic. See `docs/seo-agent-strategy-and-eval-plan.md`.

### Step 5: Run the deterministic gate
```bash
python3 -m data_sources.modules.eval.eval_runner drafts/[article-file].md --step draft
```
Exit code is the contract: `0` = ship · `2` = regenerate · `3` = block/hard-fail.
- **Exit 3 (hard-fail / block)**: a YMYL compliance violation (uncited health
  claim, missing disclaimer, or a forbidden absolute like "cures"). Do NOT
  proceed. Fix the flagged claims (cite a credible source + add a disclaimer, or
  remove the claim) and re-run. A foot-condition article can never ship with a
  hard fail.
- **Exit 2 (regenerate)**: below the step bar (70). Apply the reasons printed by
  the gate (lowest dimensions first) and re-run, up to 2 iterations.
- **Exit 0 (ship)**: proceed to the critic.

### Step 6: Independent critic (separate agent — never self-grade)
Invoke the `critic` agent (`.claude/agents/critic.md`). It reads the draft +
the deterministic scorecard and writes its verdict JSON to `/tmp/critic_<slug>.json`,
focusing on genericity, E-E-A-T, and whether citations actually support claims.
Then re-run the gate blending the critic in (it can only LOWER the score):
```bash
python3 -m data_sources.modules.eval.eval_runner drafts/[article-file].md \
    --step draft --critic /tmp/critic_<slug>.json
```
Route on the blended result exactly as in Step 5. The blended scorecard is
persisted to `data_sources/ledger/eval_log.jsonl` automatically (score saved at
this step). On any failure a regression fixture is captured automatically.

### Required metadata for the gate
Ensure the frontmatter includes **Cluster** (one of: comfortable shoes
(discovery), foot comfort education & arch support, everyday flats & loafers,
work / office comfort shoes, comfortable walking shoes (pillar)) and **Awareness
Stage** (unaware / problem-aware / solution-aware for stages 1–3) — the gate's
business-impact dimension requires both.
...
- **800 words** — hard cap. Mobile readers won't finish longer. Every paragraph must earn its place; cut anything that restates a point already made.
- Proper H1/H2/H3 hierarchy
- Primary keyword naturally integrated
- 3–5 internal links (product and collection pages — not homepage)
- 2–3 external authoritative links (only for cited claims)
- Compelling meta title and description
- Clear introduction and conclusion
- Brand voice maintained

### Structure Rules
- **Solution before education** — "what to look for" or "features that matter" must appear in the first half, not after multiple explanation sections
- **One point per concept** — never repeat the same mechanism in different words across multiple sections
- **Segment-specific CTA at close** — closing CTA must address the reader's specific context, not generic "shop now"
- **Brand voice in the first half** — at least one "At Sunnystep, we..." moment before the product section

### Proof Discipline — Four Claims That Need Evidence or Softer Language

Check every draft for these before saving:

| Claim | Overclaim (avoid) | Approved phrasing |
|---|---|---|
| Physiotherapist recommendation | "recommended by physiotherapists" | "recommended by physiotherapists we work with in Singapore" OR cite source |
| MIT Sports Lab | "MIT-validated" / "developed with MIT" | "developed in collaboration with MIT Sports Lab, which studied [specific research area]" |
| Acupressure / circulation | "stimulates circulation" | "designed to stimulate circulation" / "acupressure points that gently stimulate blood flow" |
| Blood flow / standing | "blood flow decreases" (unsourced) | "blood pools in the lower legs during prolonged standing" + cite source, OR "without the pumping action of walking, foot muscles stay contracted" |

### Engagement Requirements
- **Compelling hook** — no generic openings ("In today's world...", "Are you looking for...")
- **2–3 contextual CTAs** distributed through the article (not only at end)
- **First CTA within 500 words**
- **No paragraphs longer than 4 sentences**
- **Varied sentence rhythm** (mix short punchy + longer flowing)

### Voice Standard
- Must not read as a generic AI article — if a competitor could copy it and swap only the product names, rewrite for more brand specificity
- Bring social proof, founder truth, or customer context into the first half
- First-person brand voice at least once before the halfway point
