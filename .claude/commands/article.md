...
### Content Standards
- **800 words** — hard cap. Mobile readers won't finish longer. Cut ruthlessly: if a paragraph restates a point already made, delete it.
- Proper H1/H2/H3 hierarchy
- 3–5 internal links (product and collection pages, not homepage)
- 2–3 external authority links (cited claims only)
- Compelling hook — no generic openings
- 2–3 contextual CTAs distributed through the article
- FAQ with real user questions
- Quality score 70+

### Structure Rules
- **Solution before education** — the "what to look for" or "features that matter" section must appear in the first half of the article, not after multiple explanation sections
- **One biomechanical/scientific point per concept** — state it once, clearly. Never repeat the same mechanism in different words across multiple sections
- **Conversion by audience segment** — the closing CTA must be specific to the reader's context (e.g. "if you're a nurse… if you're a teacher…"), not a generic "shop now"
- **Brand voice in the first half** — at least one "At Sunnystep, we..." moment must appear before the product recommendation section, not only in it

### Proof Discipline — Four Claims That Require Evidence or Softened Language

Every draft must check these four claims before saving. Use the approved phrasings or cite a source:

| Claim | Overclaim (do not use) | Approved phrasing |
|---|---|---|
| Physiotherapist recommendation | "recommended by physiotherapists" | "recommended by physiotherapists we work with in Singapore" OR cite a specific source |
| MIT Sports Lab | "developed with MIT" / "MIT-validated" | "developed in collaboration with MIT Sports Lab, which studied [specific thing: pressure distribution / aerodynamics]" — always say what was studied |
| Acupressure / circulation | "stimulates circulation" / "improves blood flow" | "designed to stimulate circulation" OR "acupressure points that gently stimulate blood flow" (the design intent, not a clinical outcome claim) |
| Blood flow / standing | "blood flow decreases" (unsourced) | "blood pools in the lower legs during prolonged standing" (cite a podiatry source) OR "your foot muscles stay contracted without the pumping action of walking" |

If a source is available, cite it. If not, soften the claim to describe design intent rather than clinical outcome.

### Voice Standards
- Must not read as "well-written AI article" — test: could a competitor copy this and change only the product names? If yes, it needs more Sunnystep-specific content
- Bring in social proof, founder truth, or customer context early — not only in the product section
- First-person brand voice ("At Sunnystep, we...") at least once before the halfway point

### Differentiation Standards
- Addresses 3+ competitor gaps
- Uses real user language (not SEO-speak)
- Provides depth where competitors are thin
- Has a thesis — a specific claim that could be disagreed with, not just a topic
